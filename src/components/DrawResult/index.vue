...
export default {
  data() {
    return {
      ...
      highlightedTableId: null // Track the highlighted table ID
    };
  },
  mounted() {
    ...
    drawTablesOnCanvas() {
      ...
      this.diningTables.forEach((table) => {
        ...
        img.onload = () => {
          ...
          const desiredWidth = 80; // Set the width you want
          ...
          if (table.id === this.highlightedTableId) {
            ...
            ctx.lineWidth = 5;
            ctx.strokeRect(table.xCord, table.yCord, desiredWidth, desiredHeight);
